...
public class UnderKAndNDigits {
    ...
    public int solveIteration(ArrayList<Integer> A, int B, int C) {
        // corner case: empty array
        if (A.isEmpty()) {
            return 0;
        }
        ...
        int ans = 0;
        for (int i = 0; i < length; i++) {
            // digit n at index i is not in array A
            // all possible combination is: elements lower * digits ^ remaining slots
            ans += el.get(i) * (int) Math.pow(digits, length - i - 1);
            if (binarySearch(A, ca.get(i)) == -1) {
                break;
            }
        }
        return ans;
